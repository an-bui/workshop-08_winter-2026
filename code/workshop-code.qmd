...
[deployed html url](https://an-bui.github.io/workshop-08_winter-2026/code/workshop-code.html)
...
drought_exp_clean <- drought_exp |> 
  ...
  clean_names() |> 
  # created new column called species_name and relabelled all species with full name
  ...
  # adding new column for treatment and changing abbreviations to corresponding full names
...
ggplot(data = drought_exp_clean,
       aes(x = sla,
           y = total_g)) +
  # first layer: points to represent individual seedlings
...
This is the model prediction for the difference between Penstemon centranthifolius mass in the well watered treatment and Lotus scoparius mass in the well watered treatemnt. The model predicts that this difference is 0.05 $\pm$ 0.03 g. 

**What does `species_nameEschscholzia californica` represent?**

This is the model prediction for the difference betwen Eschscholzia californica mass in the well watered treatment and Lotus scoparius mass in the well watered treatment. The model predicts that this difference is 0.23 $\pm$ 0.03 g.
...
**Figure 4. Plant mass (g) differs across species and treatments.** Open circles represent idnividuals in each treatment (red: drought stressed, blue: well watered). Panels depict different species. Opaque points represent model predictions fof plant mass, and bars represent 95% confidence intervals of model predictions. Data source: Valliere, Justin; Zhang, Jacqueline; Sharifi, M.; Rundel, Philip (2019). Data from: Can we condition native plants to increase drought tolerance and improve restoration success? [Dataset]. Dryad. [https://doi.org/10.5061/dryad.v0861f7](https://doi.org/10.5061/dryad.v0861f7)  

Note:  

- the figure number (Figure 4) and title (Plant mass (g) differs across species and treatments) are in **bold**
- the caption includes a description of the open circles (individual observations) and opaque points (model predictions) and bars (95% CI around _predictions_) and colors (representing treatments)
- data source is copied from the data citation in section 2 ("Clean data")

## Basic components of test

**predictors**  
full model: specific leaf area (cm^2^/g), water treatment, species  
selected model: water treatment, species

**response**  
plant mass (g)

**test**  
linear model (you could also call this "multiple linear regression")
...
note:  
**7** comes from 8 estimates - 1 = 7  
**62** comes from 70 observations - 8 estimates = 62
...
**p-value**   
p < 0.001 (`p-value: < 2.2e-16` in summary)  

**significance level**  
$\alpha$ = 0.05

**reference level(s)**  
Drought stressed and _Lotus scoparius_
...
We used linear models to determine the predictive relationship between specific leaf area (cm^2^/g), treatment, and species on plant mass.
...
We found that the simplest model describing variation in plant mass included species and treatment as predictor variables (linear model: F(7, 62) = 27.08, p < 0.001, $\alpha$ = 0.05, adjusted R^2^ = 0.73).
